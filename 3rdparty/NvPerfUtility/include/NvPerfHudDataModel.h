...
namespace nv { namespace perf { namespace hud {
    ...
    public:
        ...
        Panel(const Panel& other) : Widget(Widget::Type::Panel)
        {
            ...
            {
                widgets.emplace_back(std::move(pWidget->Clone()));
            }
        }
    ...
    inline std::unique_ptr<Widget> WidgetFromYaml(const ryml::NodeRef& node, bool* pValid)
    {
        ...
        {
            return std::make_unique<ScalarText>(std::move(ScalarText::FromYaml(node, pValid)));
        }
        else if (type == "Separator")
        {
            return std::make_unique<Separator>(std::move(Separator::FromYaml(node, pValid)));
        }
        else if (type == "TimePlot")
        {
            return std::make_unique<TimePlot>(std::move(TimePlot::FromYaml(node, pValid)));
        }
        else
        {
