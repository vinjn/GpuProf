...
namespace nv { namespace perf { namespace mini_trace {
    ...
    public:
        ...
        bool Initialize(VkInstance instance, VkPhysicalDevice physicalDevice, VkDevice device)
        {
            ...
            {
                NV_PERF_LOG_ERR(20, "NVPW_VK_MiniTrace_DeviceState_Create failed, nvpaStatus = %d\n", nvpaStatus);
                return false;
            }
            m_instance = instance;
        ...
        void Reset()
        {
            if (m_isInitialized)
            {
                ...
                if (nvpaStatus != NVPA_STATUS_SUCCESS)
                {
                    NV_PERF_LOG_ERR(50, "NVPW_VK_MiniTrace_DeviceState_Destroy failed, nvpaStatus = %d\n", nvpaStatus);
                }
                m_instance = VK_NULL_HANDLE;
                m_physicalDevice = VK_NULL_HANDLE;
                m_device = VK_NULL_HANDLE;
        ...
        bool RegisterQueue(VkQueue queue)
        {
            ...
            if (nvpaStatus != NVPA_STATUS_SUCCESS)
            {
                NV_PERF_LOG_ERR(30, "NVPW_VK_MiniTrace_Queue_Register failed, nvpaStatus = %d\n", nvpaStatus);
                return false;
            }
            return true;
        }
        ...
        bool UnregisterQueue(VkQueue queue)
        {
            ...
            if (nvpaStatus != NVPA_STATUS_SUCCESS)
            {
                NV_PERF_LOG_ERR(30, "NVPW_VK_MiniTrace_Queue_Unregister failed, nvpaStatus = %d\n", nvpaStatus);
                return false;
            }
            return true;
        }
        ...
        bool FrontEndTrigger(VkCommandBuffer commandBuffer, bool useComputeMethods, TPredicateFunc&& predicateFunc)
        {
            ...
            if (nvpaStatus != NVPA_STATUS_SUCCESS)
            {
                NV_PERF_LOG_ERR(30, "NVPW_VK_MiniTrace_CommandBuffer_FrontEndTrigger failed, nvpaStatus = %d\n", nvpaStatus);
                return false;
            }
            return true;
        }
        ...
        bool MarkerCpu(VkCommandBuffer commandBuffer, const uint8_t* pUserData, size_t userDataSize, TMarkerFunc&& markerFunc)
        {
            ...
            if (nvpaStatus != NVPA_STATUS_SUCCESS)
            {
                NV_PERF_LOG_ERR(30, "NVPW_VK_MiniTrace_CommandBuffer_MarkerCpu failed, nvpaStatus = %d\n", nvpaStatus);
                return false;
            }
            return true;
        }
        ...
        bool HostTimestamp(VkCommandBuffer commandBuffer, uint32_t payload, TAddressFunc&& addressFunc)
        {
            ...
            if (nvpaStatus != NVPA_STATUS_SUCCESS)
            {
                NV_PERF_LOG_ERR(30, "NVPW_VK_MiniTrace_CommandBuffer_HostTimestamp failed, nvpaStatus = %d\n", nvpaStatus);
                return false;
            }
            return true;
        }
    ...
    public:
        ...
        bool BeginSession(VkQueue queue, uint32_t queueFamilyIndex, size_t maxFrameLatency)
        {
            ...
            auto destroyResources = ScopeExitGuard([&]() {
                ...
                vkDestroyBuffer(m_device, m_traceBuffer.buffer, nullptr);
                vkFreeMemory(m_device, m_traceBuffer.deviceMemory, nullptr);
                for (FrameResource& frameResource : m_frameResources)
                {
                    vkFreeCommandBuffers(m_device, m_commandPool, 1, &frameResource.commandBuffer);
                    vkDestroyFence(m_device, frameResource.fence, nullptr);
                }
            });

            {
                ...
                vkResult = vkCreateCommandPool(m_device, &commandPoolCreateInfo, nullptr, &m_commandPool);
            ...
            {
                {
                    ...
                    vkResult = vkCreateBuffer(m_device, &bufferCreateInfo, nullptr, &m_traceBuffer.buffer);
                    if (vkResult != VK_SUCCESS)
                    {
                ...
                }
                {
                    ...
                    vkGetBufferMemoryRequirements(m_device, m_traceBuffer.buffer, &memReqs);
                ...
                vkResult = vkBindBufferMemory(m_device, m_traceBuffer.buffer, m_traceBuffer.deviceMemory, 0);
                if (vkResult != VK_SUCCESS)
                {
            ...
            const VkDeviceAddress baseGpuVA = [&]() {
                ...
                PFN_vkGetBufferDeviceAddress pfnVkGetBufferDeviceAddress = (PFN_vkGetBufferDeviceAddress)vkGetDeviceProcAddr(m_device, "vkGetBufferDeviceAddress");
                if (pfnVkGetBufferDeviceAddress)
                {
                    return pfnVkGetBufferDeviceAddress(m_device, &bufferDeviceAddressInfo);
                }

                PFN_vkGetBufferDeviceAddressEXT pfnVkGetBufferDeviceAddressEXT = (PFN_vkGetBufferDeviceAddressEXT)vkGetDeviceProcAddr(m_device, "vkGetBufferDeviceAddressEXT");
            ...
            for (size_t frameIdx = 0; frameIdx < m_frameResources.size(); ++frameIdx)
            {
                FrameResource& frameResource = m_frameResources[frameIdx];
                {
                    ...
                    vkResult = vkAllocateCommandBuffers(m_device, &commandBufferAllocateInfo, &frameResource.commandBuffer);
                    if (vkResult != VK_SUCCESS)
                    {
                ...
                }
                {
                    ...
                    vkResult = vkCreateFence(m_device, &fenceCreateInfo, nullptr, &frameResource.fence);
                    if (vkResult != VK_SUCCESS)
                    {
        ...
        bool OnFrameEnd()
        {
            ...
            VkResult vkResult = vkGetFenceStatus(m_device, frameResource.fence);
            ...
            vkResult = vkResetFences(m_device, 1, &frameResource.fence);
            if (vkResult != VK_SUCCESS)
            {
            ...
            vkResult = vkQueueSubmit(m_queue, 1, &submitInfo, frameResource.fence);
            if (vkResult != VK_SUCCESS)
            {
        ...
        bool GetOldestFrameData(bool& isDataReady, FrameData& frameData)
        {
            ...
            VkResult vkResult = vkGetFenceStatus(m_device, oldestFrameResource.fence);
            if (vkResult == VK_ERROR_DEVICE_LOST)
            {
                NV_PERF_LOG_ERR(50, "Device is lost\n");
                return false;
            ...
            isDataReady = true;
            {
                ...
                vkResult = vkMapMemory(m_device, m_traceBuffer.deviceMemory, m_perFrameBufferSize * m_getFrameIdx, m_perFrameBufferSize, 0, (void**)&pReport);
                auto unmap = ScopeExitGuard([&]() {
                    vkUnmapMemory(m_device, m_traceBuffer.deviceMemory);
                });
